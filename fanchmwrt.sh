...
# 主源码：OpenWrt官方，集成fanchmwrt界面、Nikki插件、Docker、IPsec/L2TP、磁盘分区工具
...
# 添加fanchmwrt界面/组件源（补充界面和自定义组件）
echo "src-git fanchmwrt https://github.com/fanchmwrt/fanchmwrt.git;openwrt-24.10" >> "feeds.conf.default"

# 更新并安装基础软件源
./scripts/feeds update -a
./scripts/feeds install -a

# 集成磁盘管理工具组件（从immortalwrt同步）
cp -r -n ../immortalwrt/feeds/luci/applications/luci-app-diskman/* ./feeds/luci/applications/luci-app-diskman/ 2>/dev/null

# 集成kenzo和small软件源（保障PassWall/HomeProxy等插件）
