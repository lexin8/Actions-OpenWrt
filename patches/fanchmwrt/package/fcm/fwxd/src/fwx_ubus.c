...
struct json_object *fwx_api_get_dashboard_param(struct json_object *req_obj) {
    ...
    char monitor_device[64] = {0};
    char exclude_nets[2048] = {0};
    char exclude_proto_mask[32] = {0};
    char exclude_lan_lan[8] = {0};
    int ret = fwx_uci_get_value(uci_ctx, "fwx.dashboard.monitor_device", monitor_device, sizeof(monitor_device));
    if (ret != 0) {
        monitor_device[0] = '\0';
    }
    if (fwx_uci_get_value(uci_ctx, "fwx.traffic_stat.exclude_lan_lan", exclude_lan_lan, sizeof(exclude_lan_lan)) != 0)
        strcpy(exclude_lan_lan, "0");
    if (fwx_uci_get_value(uci_ctx, "fwx.traffic_stat.exclude_proto_mask", exclude_proto_mask, sizeof(exclude_proto_mask)) != 0)
        strcpy(exclude_proto_mask, "0");
    if (fwx_uci_get_value(uci_ctx, "fwx.traffic_stat.exclude_nets", exclude_nets, sizeof(exclude_nets)) != 0)
        exclude_nets[0] = '\0';
    ...
    json_object_object_add(data_obj, "exclude_lan_lan", json_object_new_string(exclude_lan_lan));
    json_object_object_add(data_obj, "exclude_proto_mask", json_object_new_string(exclude_proto_mask));
    json_object_object_add(data_obj, "exclude_nets", json_object_new_string(exclude_nets));
...
struct json_object *fwx_api_set_dashboard_param(struct json_object *req_obj) {
    ...
    {
        struct json_object *o = json_object_object_get(req_obj, "exclude_lan_lan");
        if (o) {
            char tmp[8] = {0};
            if (json_object_get_type(o) == json_type_string) {
                ...
                snprintf(tmp, sizeof(tmp), "%d", json_object_get_int(o));
            fwx_uci_set_value(uci_ctx, "fwx.traffic_stat.exclude_lan_lan", tmp);
        }
    }
    {
        struct json_object *o = json_object_object_get(req_obj, "exclude_proto_mask");
        if (o) {
            char tmp[32] = {0};
            if (json_object_get_type(o) == json_type_string) {
                const char *s = json_object_get_string(o);
                ...
                snprintf(tmp, sizeof(tmp), "%u", (unsigned int)json_object_get_int(o));
            fwx_uci_set_value(uci_ctx, "fwx.traffic_stat.exclude_proto_mask", tmp);
        }
    }
    {
        struct json_object *o = json_object_object_get(req_obj, "exclude_nets");
        if (o) {
            if (json_object_get_type(o) == json_type_string) {
                const char *s = json_object_get_string(o);
                if (s)
                    fwx_uci_set_value(uci_ctx, "fwx.traffic_stat.exclude_nets", (char *)s);
            }
        }
    }
