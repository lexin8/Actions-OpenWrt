...
void fwx_apply_traffic_stat_config(void)
{
	...
	struct json_object *root, *data, *arr;
	const char *jstr;
	char *msg;
	char *p;
	char *saveptr;
	...
	if (fwx_uci_get_value(ctx, "fwx.traffic_stat.exclude_lan_lan", buf, sizeof(buf)) == 0 && buf[0])
		exclude_lan = atoi(buf);

	if (fwx_uci_get_value(ctx, "fwx.traffic_stat.exclude_proto_mask", buf, sizeof(buf)) == 0 && buf[0])
		proto_mask = (unsigned int)strtoul(buf, NULL, 10);

	nets_buf[0] = '\0';
	fwx_uci_get_value(ctx, "fwx.traffic_stat.exclude_nets", nets_buf, sizeof(nets_buf));

	uci_free_context(ctx);

	root = json_object_new_object();
	data = json_object_new_object();
	json_object_object_add(root, "api", json_object_new_string("set_traffic_stat_exclude"));
	json_object_object_add(data, "exclude_lan_lan", json_object_new_int(exclude_lan));
	json_object_object_add(data, "exclude_proto_mask", json_object_new_int((int)proto_mask));
	...
	while (p && *p) {
		...
		while (*p && *p != ' ' && *p != '\t' && *p != '\n')
			p++;
		len = (size_t)(p - saveptr);
		if (len == 0)
			continue;
		if (len >= sizeof(one))
			len = sizeof(one) - 1;
		memcpy(one, saveptr, len);
		one[len] = '\0';
		json_object_array_add(arr, json_object_new_string(one));
	}
	json_object_object_add(data, "exclude_nets", arr);
	json_object_object_add(root, "data", data);

	jstr = json_object_to_json_string_ext(root, JSON_C_TO_STRING_PLAIN);
	if (jstr) {
		msg = malloc(strlen(jstr) + 1);
		if (msg) {
			strcpy(msg, jstr);
			fwx_send_msg_to_kernel(msg);
			free(msg);
		}
	}
	json_object_put(root);

	update_fwx_proc_u32_value("stat_exclude_proto_mask", proto_mask);
	sprintf(buf, "%d", exclude_lan);
	update_fwx_proc_value("stat_exclude_lan_lan", buf);
}
